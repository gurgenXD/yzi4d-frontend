export async function getSpecialists(searchParams: URLSearchParams) {
  const specialists = await fetch(
    `${process.env.NEXT_PUBLIC_YZI4D_HOST}/specialists?${searchParams.toString()}`,
    { cache: "no-store" }
  ).then((res) => res.json());
...
export async function getSpecializations() {
  const specializations = await fetch(
    `${process.env.NEXT_PUBLIC_YZI4D_HOST}/specialists/specializations`,
    { cache: "no-store" }
  ).then((res) => res.json());
...
export async function getSpecialist(id: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_YZI4D_HOST}/specialists/${id}`, {
    cache: "no-store",
  });
...
export async function getSpecialistServices(id: string, searchParams: URLSearchParams) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_YZI4D_HOST}/specialists/${id}/services?${searchParams.toString()}`,
    { cache: "no-store" }
  );
...
export async function getService(id: string, catalog_type: string) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_YZI4D_HOST}/catalog/${catalog_type}/services/${id}`,
    { cache: "no-store" }
  );
...
export async function getPromotions() {
  const promotions = await fetch(`${process.env.NEXT_PUBLIC_YZI4D_HOST}/promotions`, {
    cache: "no-store",
  }).then((res) => res.json());
...
export async function getDocumentsCategories() {
  const categories = await fetch(`${process.env.NEXT_PUBLIC_YZI4D_HOST}/documents`, {
    cache: "no-store",
  }).then((res) => res.json());
