...
export function useGetServices(category_id: string | null, catalog_type: string, search_query: string, page: number) {
    const fetcher = async (url: any) => await fetch(url).then(res => res.json())

    const params = `?page=${page}` + (category_id ? `&category_id=${category_id}` : "") + (search_query ? `&search_query=${search_query}` : "")
    const { data, error, isLoading } = useSWR(
