...
function formatPatientName(patient: any) {
    return `${patient.surname} ${patient.name} ${patient.patronymic ? `${patient.patronymic[0]}.` : null}`
}
...
export default function PatientBlock({ patient, isLoading, error }: { patient: any, isLoading: boolean, error: boolean }) {
                ...
                ? <li className="nav-item has-submenu">
                    ...
                    <ul className="submenu collapse show">
                        {patient.members.map((member: any) => (
                            <li key={member.id}><a className="nav-link" href={`/profile/${member.id}`}>{formatMemberName(member.full_name)}</a></li>
                        ))}
                    </ul>
                </li>
