  ...
  if (filialFilter) {
    ...
    checkboxAnyFilial.addEventListener("change", function () {
      if (this.checked) {
        ...
        filialFilterVal.textContent = document.querySelector(
          "label[for=appointmentFilialFilter1]"
        ).textContent;
      }
    });
    ...
    checkboxes.forEach((checkbox, index) => {
      checkbox.addEventListener("change", function () {
        ...
        const selectedCheckboxes = Array.from(checkboxes).filter(
          (checkbox, index) => index !== 0 && checkbox.checked
        );
        const selectedCount = selectedCheckboxes.length;

        filialFilterVal.textContent =
          selectedCount === 0 ? "Любой филиал" : `Выбрано филиалов: ${selectedCount}`;
      });
    });
  }
}
